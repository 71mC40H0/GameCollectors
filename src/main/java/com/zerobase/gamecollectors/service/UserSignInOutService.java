...
@Slf4j
public class UserSignInOutService {
    ...
    private static final String REFRESH_TOKEN_PREFIX = "u_rtk:";
    private static final String BLACKLIST_PREFIX = "u_blacklist:";

    @Transactional
    public TokenDto signIn(SignInServiceDto serviceDto) {
        User u = userRepository.findByEmail(serviceDto.getEmail())
            .orElseThrow(() -> new CustomException(ErrorCode.NOT_FOUND_USER));

        if (!passwordEncoder.matches(serviceDto.getPassword(), u.getPassword())) {
            throw new CustomException(ErrorCode.MISMATCHED_PASSWORD);
        }

        return TokenDto.builder()
            .accessToken(provider.createAccessToken(u.getEmail(), u.getId(), UserType.USER))
            .refreshToken(provider.createRefreshToken(u.getEmail(), u.getId(), UserType.USER))
            .build();
    }

    @Transactional
    public TokenDto reissue(String refreshToken) {
        if (!provider.validateToken(refreshToken, UserType.USER).equals(TokenType.REFRESH_TOKEN)) {
            throw new CustomException(ErrorCode.INVALID_REFRESH_TOKEN);
        }

        UserVo userVo = provider.getUserVo(refreshToken);
        User u = userRepository.findByIdAndEmail(userVo.getId(), userVo.getEmail())
            .orElseThrow(() -> new CustomException(ErrorCode.NOT_FOUND_USER));
        ...
        return TokenDto.builder()
            ...
            .refreshToken(provider.getExpiration(refreshToken) - now.getTime() < REFRESH_EXPIRE_TIME
                ? provider.createRefreshToken(u.getEmail(), u.getId(), UserType.USER) : refreshToken)
            .build();
    }

    @Transactional
    public void signOut(String accessToken) {
        if (!provider.validateToken(accessToken, UserType.USER).equals(TokenType.ACCESS_TOKEN)) {
        ...
        RedisUtil.deleteData(REFRESH_TOKEN_PREFIX + u.getEmail());
        log.info("Sign Out User -> id: {}", u.getId());
    }
